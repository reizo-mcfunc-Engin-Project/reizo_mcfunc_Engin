...
return run function reizo_libs:abs/get/early_return/0

# 1以上でも終わり。
execute \
if score $AbsGet.In reizo_mcfunc_Engin.Temp matches 1.. run \
return run function reizo_libs:abs/get/early_return/1..

# 文字列に変換
function reizo_libs:abs/get/conversion/string.m with storage reizo_mcfunc_engin:lib In

# 絶対値を求める。
data modify storage reizo_mcfunc_engin:lib Temp.AbsGet set string storage reizo_mcfunc_engin:lib Temp.AbsGet 1

# 求めたら数値に変換
function reizo_libs:abs/get/conversion/num.m with storage reizo_mcfunc_engin:lib Temp
