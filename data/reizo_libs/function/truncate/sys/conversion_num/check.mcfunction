#> reizo_libs:truncate/sys/conversion_num/check
#
# 
#
...
#> ノーマルかNullか検知
    ...
    return run function reizo_libs:truncate/sys/conversion_num/normal.m with storage reizo_mcfunc_engin:lib Truncate
    # Null
    execute \
    if data storage reizo_mcfunc_engin:lib {Return:{CheckPlaceValue:"Null"}} run \
    return run function reizo_libs:truncate/sys/conversion_num/null.m with storage reizo_mcfunc_engin:lib Truncate