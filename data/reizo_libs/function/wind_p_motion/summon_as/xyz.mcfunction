#> reizo_libs:wind_p_motion/summon_as/xyz
#
# 
#
# @within function reizo_libs:wind_p_motion/xyz
...
#> 正の数か負の数かチェックからの実行
    # X
        # Signum
        function reizo_libs:signum_check/score {Selecter:"$x",Obj:"reizo_mcfunc_Engin.Temp"}
        # 正の値
        execute \
        ...
        function reizo_libs:wind_p_motion/summon_as/summon/xyz/x
        # 負の値
        execute \
        if data storage reizo_mcfunc_engin:lib Return{Signum:-1.0f} \
        as @a[tag=reizo_mcfunc_Engin.Wind_P_Motion.This] at @s \
    ...
    # Y
        # Signum
        function reizo_libs:signum_check/score {Selecter:"$y",Obj:"reizo_mcfunc_Engin.Temp"}
        # 正の値
        execute \
        if data storage reizo_mcfunc_engin:lib Return{Signum:1.0f} \
        as @a[tag=reizo_mcfunc_Engin.Wind_P_Motion.This] at @s \
        positioned ~ ~-0.733 ~ run \
        function reizo_libs:wind_p_motion/summon_as/summon/xyz/y
        # 負の値
        execute \
        if data storage reizo_mcfunc_engin:lib Return{Signum:-1.0f} \
        as @a[tag=reizo_mcfunc_Engin.Wind_P_Motion.This] at @s \
