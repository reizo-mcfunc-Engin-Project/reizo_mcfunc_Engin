...
# API用変数に値をコピー
    ...
    execute if entity @s[type=player] run data modify storage reizo_mcfunc_engin:api Args.Extends.foreach set from storage reizo_mcfunc_engin:_ _.Registry.Extends
    # Final情報
    ...
    # execute if entity @s[type=player] run data modify storage reizo_mcfunc_engin:api Args.Extends.foreach set from storage reizo_mcfunc_engin:_ _.Registry.Final
