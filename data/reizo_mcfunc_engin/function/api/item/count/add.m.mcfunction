#> reizo_mcfunc_engin:api/item/count/add.m
#
# アイテムの個数を指定した値分addする。
#
# @api
...
# 現在のCountから指定した値分減らす
    ...
    # 引数分減らしてストレージに保存
    $execute store result storage reizo_mcfunc_engin:api Args.Item.Count.Value int 1 run scoreboard players add $API.Item.Count reizo_mcfunc_Engin.Temp $(Value)
    # お掃除
