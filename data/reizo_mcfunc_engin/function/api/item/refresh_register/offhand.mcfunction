#> reizo_mcfunc_engin:api/item/refresh_register/mainhand
#
# オフハンドにあるアイテムの登録されている情報を新しくする。
# !注意!: Fieldも新しいものに初期化するので今持っているFieldを持ち越すことはできません。
#
# @within function reizo_mcfunc_engin:api/item/refresh_register_all

# 既存にargsが存在する可能性があるため、退避する。
function reizo_mcfunc_engin:asset/.manager/common/context/args/stash

# 引数の取得
data modify storage reizo_mcfunc_engin:context Args set from entity @s equipment.offhand.components."minecraft:custom_data".Item.Args
