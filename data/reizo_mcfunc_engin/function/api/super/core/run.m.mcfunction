...
# execute unless predicate {condition:"value_check",value:{type:"storage",storage:"reizo_mcfunc_engin:api",path:"Super.RUN.required"},range:1b} run tellraw @a {storage:"reizo_mcfunc_engin:api",nbt:"Super"}
