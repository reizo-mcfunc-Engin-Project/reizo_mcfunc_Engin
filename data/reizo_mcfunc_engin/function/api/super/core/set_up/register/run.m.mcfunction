...
# 反転
    ...
    # 継承情報が残り一つなら配列を反転する。
    execute if score $API.Super.SetUp.Register.Len reizo_mcfunc_Engin.Temp matches 1 run function reizo_mcfunc_engin:api/super/core/set_up/register/reverse.m with storage reizo_mcfunc_engin:api Super.data
    # お掃除
