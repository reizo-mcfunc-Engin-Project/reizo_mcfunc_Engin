...
# 先頭取得
data modify storage reizo_mcfunc_engin:item Rerun.Entry set from storage reizo_mcfunc_engin:item Old.Success.Inventory.NotHold[0]

# 共通処理
function reizo_mcfunc_engin:asset/item/.manager/not_hold/rerun/_common

# 要素の削除
data remove storage reizo_mcfunc_engin:item Old.Success.Inventory.NotHold[0]
