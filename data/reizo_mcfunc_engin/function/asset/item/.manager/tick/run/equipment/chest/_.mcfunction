...
#> 実行チェック
    ...
    # chest
    execute \
    unless data storage reizo_mcfunc_engin:item data.Equipment.chest.components."minecraft:custom_data".Item.Registry{IsTicking:0b} \
    if data storage reizo_mcfunc_engin:item data.Equipment.chest.components."minecraft:custom_data".Item.Registry.Slot[{Value:-4}] run \
