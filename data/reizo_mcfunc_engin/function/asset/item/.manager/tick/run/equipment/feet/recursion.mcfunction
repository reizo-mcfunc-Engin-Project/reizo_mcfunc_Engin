#> reizo_mcfunc_engin:asset/item/.manager/tick/run/equipment/feet/recursion
#
# 
#
...
# お掃除
data remove storage reizo_mcfunc_engin:item data.Equipment.feet.components."minecraft:custom_data".Item.Registry.Slot[0]

# NotHold関係
execute \
if data storage reizo_mcfunc_engin:item data.Equipment.feet.components."minecraft:custom_data".Item.Registry{NotHold:1b} \
unless data storage reizo_mcfunc_engin:item data.Equipment.feet.components."minecraft:custom_data".Item.Registry.Slot[0] run \
...
# 先頭が残るなら再帰
execute \
if data storage reizo_mcfunc_engin:item data.Equipment.feet.components."minecraft:custom_data".Item.Registry.Slot[0] run \
