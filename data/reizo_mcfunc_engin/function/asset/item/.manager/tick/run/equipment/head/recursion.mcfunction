#> reizo_mcfunc_engin:asset/item/.manager/tick/run/equipment/head/recursion
#
# 
#
...
# お掃除
data remove storage reizo_mcfunc_engin:item data.Equipment.head.components."minecraft:custom_data".Item.Registry.Slot[0]
...
# 先頭が残るなら再帰
execute \
if data storage reizo_mcfunc_engin:item data.Equipment.head.components."minecraft:custom_data".Item.Registry.Slot[0] run \
