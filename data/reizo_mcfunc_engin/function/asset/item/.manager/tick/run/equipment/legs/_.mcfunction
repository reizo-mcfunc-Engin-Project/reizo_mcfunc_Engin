...
# スコアにも入れておく
scoreboard players set $Item.Slot reizo_mcfunc_Engin.Temp -5
...
#> 実行チェック
    ...
    # legs
    execute \
    unless data storage reizo_mcfunc_engin:item data.Equipment.legs.components."minecraft:custom_data".Item.Registry{IsTicking:0b} run \
