...
# Oldのデータが現在のデータと一致したら再度同じクラスの関数を実行しに行く。
    ...
    execute if data storage reizo_mcfunc_engin:item {IsRerun:0b} if data storage reizo_mcfunc_engin:item Old.Success.Inventory.Tick[0] run return run function reizo_mcfunc_engin:asset/item/.manager/tick/rerun/inventory
